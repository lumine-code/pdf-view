...
class PdfScrollmap {
  ...
  addViewer(viewer) {
    ...
    const context = {
      ...
      lastData: null,
      itemVisible: this.isItemVisible(viewer),
    };
    ...
    // The strip is the iframe's sibling, not its child -- nothing can go
    // inside an iframe -- so the pane's tab switch, which hides only the item
    // view itself, leaves the strip painted over whatever tab comes next.
    // Mirror the exact mechanism instead: the pane hides an item by writing
    // 'display: none' onto its view, so watch the iframe's style attribute.
    const visibilityObserver = new MutationObserver(() => {
      const visible = this.isItemVisible(viewer);
      if (visible === context.itemVisible) {
        return;
      }
      context.itemVisible = visible;
      this.renderViewer(context);
    });
    visibilityObserver.observe(viewer.element, {
      attributes: true,
      attributeFilter: ["style"],
    });
    context.subscriptions.add(new Disposable(() => visibilityObserver.disconnect()));
  }
  ...
  renderViewer(context) {
    ...
    if (!this.enabled || !data?.items || !this.isItemVisible(context.viewer)) {
  ...
  isItemVisible(viewer) {
    return viewer.element?.style.display !== "none";
  }
