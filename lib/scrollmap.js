...
class PdfScrollmap {
    ...
    context.subscriptions.add(
      lumine.workspace.onDidChangeActivePaneItem(() => {
        if (lumine.workspace.getActivePaneItem() === viewer && context.lastData) {
          viewer.element.ownerDocument.defaultView.requestAnimationFrame(() =>
            this.renderViewer(context),
          );
        }
      }),
    );
    if (typeof lumine.workspace.observePaneItemSurface === "function") {
      let initial = true;
      context.subscriptions.add(
        lumine.workspace.observePaneItemSurface(viewer, () => {
          if (initial) {
            initial = false;
            return;
          }
          context.simplemap?.destroy();
          context.simplemap = null;
          this.ensureSimplemap(context);
          if (context.lastData) this.renderViewer(context);
        }),
      );
    }
  }
  ...
  ensureSimplemap(context) {
    ...
    context.simplemap = new this.SimplemapClass(context.viewer.element.ownerDocument);
