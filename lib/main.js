...
module.exports = {
  ...
  activate() {
    ...
    this.disposables = new CompositeDisposable(
      ...
      // scope every one of its twenty-four items did nothing unless the iframe
      // itself had focus. The keymap still binds them on `.pdf-view`, so a
      // keystroke reaches the viewer it came from either way.
      atom.commands.add("atom-workspace", this.viewerCommands()),
    );
  },
   ...
   * @returns {Object} a command map for `atom.commands.add`
   */
  viewerCommands() {
    const own = {
